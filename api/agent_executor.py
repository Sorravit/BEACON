...
class AgentExecutor:
    ...
    async def _phase_plan(self, task: Task, is_retry: bool = False,
        ...
        prompt = (
            ...
            "SERVER / DAEMON LAUNCH RULE — CRITICAL:\n"
            "- Any step that starts a long-lived/never-terminating process (web server,\n"
            "  ASGI/WSGI server, HTTP server, dev watcher, daemon) MUST use delegate_background_task.\n"
            "  Examples: uvicorn, gunicorn, flask run, manage.py runserver,\n"
            "            python -m http.server, npm start, npm run dev, yarn dev,\n"
            "            pnpm dev, next dev, serve, nohup ...\n"
            "- Ordinary one-shot commands (pip install, pytest, build, grep, etc.)\n"
            "  keep using execute_command or execute_long_command as normal.\n\n"
